...
> **⚠️ Nunca suba o arquivo `.env` real para o Git.** Ele já está no `.gitignore`.
...
### ⚠️ Usuário de demonstração (só no frontend)
...
Em produção, remova o bloco marcado com `⚠️ BLOCO DE DEMO` no `login.js`.
...
Nos arquivos HTML, procure pelo comentário `✏️ PERSONALIZE` e substitua o bloco:
